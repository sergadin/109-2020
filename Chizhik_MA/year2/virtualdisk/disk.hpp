...
class VirtualDisk {
	...
	private:
		...
		// В файловой записи записываются нулевая длина, даты создания и последнего изменения, равные текущей, тип type
		// (если type = DIR_T, в файле создаются записи . и ..); выделяется новый кластер, и его индекс записывается в поле индекса первого кластера
	...
	public:
		...
		// Минимальный размер диска - 2 КБ, кластера - 512 байт; размер кластера при этом должен быть кратен 512 байтам

...
class BitIntSet {
	...
	public:
		...
		// Первый оператор вызывается для константных ссылок, второй - в случае, если обе ссылки неконстантны (второй, в отличие от первого, имеет возможность закэшировать все элементы)
		friend bool operator==(const BitIntSet& A, const BitIntSet& B);
